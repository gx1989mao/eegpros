...
sub = 4;
subplot(sub,1,1);
plot(x,judge_ins(1,:));hold on;
plot([0,LEN-win_width ],[judge(1,epo),judge(1,epo)],'r-'); hold on;
subplot(sub,1,2);
...
%         if sigma>2*judge(4,epo)
%             yy(p) = 1;
%             if p>1
%                 if yy(p-1)==1;%% kick out the next one
%                     yy(p) = 0;
%                 end
%             end
%         end
%         if sigma>1.5*judge(4,epo)%% larger than 2 times of average Sigma of 30s epoch
...
%         end
%     end
%     ss_num_all_epoch(1,epo) = sum(yy);
%     Sig = rec(1,(epo-1)*LEN+1:epo*LEN); 
%     if max(abs(Sig))>100e-6;
%         ss_num_all_epoch(1,epo) = -1;
%     end
% end
% toc;
% 
